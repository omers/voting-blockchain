...
def get_open_surveys():
    ...
    if len(blockchain.chain) < length and blockchain.check_chain_validity(
        longest_chain.chain
    ):
        ...
        for block in longest_chain.chain:
            if not compute_open_surveys(
                block, longest_chain.open_surveys, longest_chain.chain_code
            ):
    ...
    return jsonify({"length": len(blockchain.open_surveys), "surveys": list(surveys)})
...
def validate_transaction(transaction):
    ...
    if transaction["type"].lower() == "open":
        ...
        questionid = transaction["content"]["questionid"]
        if (
            questionid in blockchain.open_surveys
            and blockchain.open_surveys[questionid]["author"]
            == transaction["content"]["author"]
            and blockchain.open_surveys[questionid]["status"] == "opening"
        ):
        ...
        try:
            exec(
                transaction["content"]["code"],
                blockchain.chain_code,
                blockchain.chain_code,
            )
            return True
        except:
        ...
        try:
            thread = threading.Thread(
                target=blockchain.chain_code[transaction["content"]["contract"]],
                args=transaction["content"]["arguments"],
            )
            thread.start()
            return True
        except:
            print(
                "Error when execute chain_code {}".format(
...
def compute_open_surveys(block, open_surveys, chain_code):
    for transaction in block.transactions:
        ...
        if transaction["type"].lower() == "open":
            ...
            questionid = transaction["content"]["questionid"]
            if (
                questionid in open_surveys
                and open_surveys[questionid]["author"]
                == transaction["content"]["author"]
                and open_surveys[questionid]["status"] == "opening"
            ):
